...
?   Lookup Tables & State Machines
    ...
    * Lookup Table
    ...
    - Often called dictionaries, Objects work like dictionaries in JavaScript, holding key value pairs to store collections of data.
    ...
    - Default evaluation does not need to wait for all conditions to get processed. If a key does not exist in the lookup table, it returns the default condition straight-away.
...
//   console.log(stateAbbreviation["hawaii"].toUpperCase());
...
// function howIsChild with parameter name of state that returns messages based on the kid's status.
const howIsChild = (state) => {
    if (state == "Hungry") {
        return "THE KID WANTS FOOD, WHERE'S THE SNACK?!?";
    } else if (state == "Sad") {
...
const kidFeelsTable = {
    Hungry: "THE KID WANTS FOOD, WHERE'S THE SNACK?!?",
...
// Creates a function to handle and transition between states
