...
// Create a function to show how our password is being used/encrypted (just a demo here)
// const testingBcrypt = (password) => {
//   // Storing in the variable name encrypt the ability to use bcrypt's hashSync method
...
/* 
  ...
  {
    "email": "littlekittycat@kittyemail.com"
    "password": "12345"
  }
*/

router.post("/login", async (req, res) => {
  // res.send(req.body.email); //used to test that the endpoint is working.
  try {
    //1. Capture data provided by user (request body).
    const {email, password} = req.body;
    //2. Check database to see if email supplied exists
      // * .findOne() is a MongoDB method that accepts a query as an argument. Returns an instance of a document that matches.
      ...
      if (!user) throw new Error("User not found.") // Really, use "Email or password does not match."
    //3. If email exists, consider if password matches (decrypt).
      ...
      // Return a true or false value, compare (string, hashed) and return true/false
