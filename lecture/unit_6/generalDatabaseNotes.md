...
Non-Relational: // More scalable and faster - Has 5 Types of Structure: document data store, column-oriented database, key-value store, document store, and graph database - Flexible Structure - Uses a Primary Key (ID) to give individuality to each document - Data is stored as documents containing JSON object - Ex: MongoDB, Apache Cassandra, Couchbase
...
- Uses hashing and salt to hide the password/value so it's nearly impossible to decrypt (hack/solve encryption)
